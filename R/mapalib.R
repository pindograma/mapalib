...
get_map_data = function(year, cities, epsg, aggregate_fun, ..., source_order = NULL, districts = NULL) {
  purrr::map_dfr(cities, function(city_id) {
    ...
    spt = data %>%
      ...
      dplyr::summarize(rn = first(rn), cand = mean(cand)) %>%
